...
export async function POST(request: Request) {
  try {
    ...
    const { tanggal, deskripsi, referensi, tipeReferensi, periodeId, details } = body

    // Validate required fields
    if (!tanggal || !deskripsi || !periodeId || !details || details.length === 0) {
      return NextResponse.json({ error: "Missing required fields" }, { status: 400 })
    }

    // Validate double-entry: total debit should equal total credit
    const totalDebit = details.reduce((sum: number, detail: any) => sum + (detail.debit || 0), 0)
    const totalCredit = details.reduce((sum: number, detail: any) => sum + (detail.kredit || 0), 0)

    if (Math.abs(totalDebit - totalCredit) > 0.01) { // Allow small floating point differences
      return NextResponse.json({
    ...
    if (!periode || !periode.isActive) {
      return NextResponse.json({ error: "Invalid or inactive accounting period" }, { status: 400 })
    }
