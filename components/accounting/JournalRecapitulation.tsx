...
import { JurnalEntry, AccumulationPeriod, AccumulationData } from "@/types/accounting"
...
export function JournalRecapitulation({ selectedPeriode, className }: JournalRecapitulationProps) {
  const [accumulationPeriod, setAccumulationPeriod] = useState<AccumulationPeriod>("monthly")
  const [startDate, setStartDate] = useState("")
  const [endDate, setEndDate] = useState("")
  const [selectedPeriodData, setSelectedPeriodData] = useState<AccumulationData | null>(null)
  ...
  const exportDetailedMonthlyReport = async () => {
    ...
    // Title
    pdf.setFontSize(20)
    pdf.text('Rekapitulasi Jurnal Bulanan Detail', 105, 20, { align: 'center' })
    ...
    if (startDate && endDate) {
      pdf.text(`Tanggal: ${new Date(startDate).toLocaleDateString('id-ID')} - ${new Date(endDate).toLocaleDateString('id-ID')}`, 20, 45)
    }
    ...
    for (const monthData of accumulationData) {
      ...
      pdf.text(`${formatPeriodLabel(monthData.period)} - Total: Rp ${monthData.totalDebit.toLocaleString('id-ID')}`, 20, yPosition)
      yPosition += 10
      ...
      autoTable(pdf, {
        head: [['Tanggal', 'Nomor Jurnal', 'Deskripsi', 'Total Debit', 'Total Kredit', 'Status']],
    ...
    pdf.setFontSize(12)
    pdf.setFont('helvetica', 'bold')
    pdf.text(`Total Keseluruhan - Debit: Rp ${totalSummary.totalDebit.toLocaleString('id-ID')}, Kredit: Rp ${totalSummary.totalKredit.toLocaleString('id-ID')}, Transaksi: ${totalSummary.transactionCount}`, 20, yPosition)
    ...
    // Save the PDF
    pdf.save(`rekapitulasi-bulanan-detail-${new Date().toISOString().split('T')[0]}.pdf`)
  }
  ...
  const exportDetailedYearlyReport = async () => {
    ...
    // Title
    pdf.setFontSize(20)
    pdf.text('Rekapitulasi Jurnal Tahunan Detail', 105, 20, { align: 'center' })
    ...
    for (const yearData of accumulationData) {
      ...
      pdf.setFontSize(14)
      pdf.setFont('helvetica', 'bold')
      pdf.text(`${formatPeriodLabel(yearData.period)} - Total: Rp ${yearData.totalDebit.toLocaleString('id-ID')}`, 20, yPosition)
      yPosition += 10
      ...
      autoTable(pdf, {
        head: [['Bulan', 'Total Debit', 'Total Kredit', 'Jumlah Transaksi', 'Status']],
    ...
    // Save the PDF
    pdf.save(`rekapitulasi-tahunan-detail-${accumulationData[0]?.period || 'unknown'}.pdf`)
  }
  ...
  const exportDetailedDailyReport = async () => {
    ...
    for (const dayData of accumulationData) {
      ...
      pdf.setFontSize(14)
      pdf.setFont('helvetica', 'bold')
      pdf.text(`${formatPeriodLabel(dayData.period)} - Total: Rp ${dayData.totalDebit.toLocaleString('id-ID')}`, 20, yPosition)
      yPosition += 10
      ...
      const entriesTableData = dayEntries.map((entry: JurnalEntry) => {
        const totalDebit = entry.details.reduce((sum: number, detail) => sum + detail.debit, 0)
        const totalKredit = entry.details.reduce((sum: number, detail) => sum + detail.kredit, 0)
      ...
      autoTable(pdf, {
        head: [['Waktu', 'Nomor Jurnal', 'Deskripsi', 'Total Debit', 'Total Kredit', 'Status']],
        body: entriesTableData,
        startY: yPosition,
        styles: {
          fontSize: 7,
    ...
    // Save the PDF
    pdf.save(`rekapitulasi-harian-detail-${new Date().toISOString().split('T')[0]}.pdf`)
  }
  ...
  const togglePeriodExpansion = (periodStr: string) => {
    ...
    const getMonthlyAccumulationForYear = (yearPeriod: string): AccumulationData[] => {
    ...
    yearEntries.forEach(entry => {
      ...
      monthData.totalDebit += entry.details.reduce((sum, detail) => sum + detail.debit, 0)
      monthData.totalKredit += entry.details.reduce((sum, detail) => sum + detail.kredit, 0)
  ...
  const getDailyEntriesForMonth = (monthPeriod: string): JurnalEntry[] => {
    ...
    return entries.filter(entry => {
      const entryDate = new Date(entry.tanggal)
      return entryDate.getFullYear() === yearNum && entryDate.getMonth() === monthNum
    }).sort((a, b) => new Date(a.tanggal).getTime() - new Date(b.tanggal).getTime())
  }
    ...
    <div className={className}>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="w-full sm:w-48">
              ...
              <Select value={accumulationPeriod} onValueChange={handlePeriodChange}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <div className="space-y-4">
            <Label className="text-base font-medium">Rekapitulasi per Periode</Label>
            {accumulationLoading ? (
              <div className="flex items-center justify-center py-8">
                <div className="text-sm text-muted-foreground">Memuat data rekapitulasi...</div>
              </div>
                ...
                <Table>
                  ...
                  <TableBody>
                        ...
                        <React.Fragment key={period.period}>
                          ...
                          <TableRow className="hover:bg-muted/50">
                            ...
                            <TableCell className="text-center">
                              <Badge variant={period.isBalanced ? "default" : "destructive"}>
                                {period.isBalanced ? "Seimbang" : "Tidak Seimbang"}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-right">
                              <Button
                          ...
                          {isExpanded && (() => {
                            if (accumulationPeriod === 'yearly') {
                              ...
                              const monthlyData = getMonthlyAccumulationForYear(period.period)
                              return monthlyData.map((monthData) => (
                                <TableRow key={monthData.period} className="bg-muted/20">
                                  <TableCell className="pl-8 font-normal text-sm">
                                    {formatPeriodLabel(monthData.period)}
                                  </TableCell>
                                  <TableCell className="text-right text-sm">
                                    Rp {monthData.totalDebit.toLocaleString('id-ID')}
                                  </TableCell>
                                  <TableCell className="text-right text-sm">
                                    Rp {monthData.totalKredit.toLocaleString('id-ID')}
                                  </TableCell>
                                  <TableCell className="text-right text-sm">
                                    {monthData.transactionCount}
                                  </TableCell>
                                  <TableCell className="text-center">
                                    <Badge variant={monthData.isBalanced ? "default" : "destructive"} className="text-xs">
                                  ...
                                  <TableCell className="text-right">
                                    <Button
                                      variant="ghost"
                                      size="sm"
                                      onClick={() => handleViewDetails(monthData)}
                                    >
                              ...
                              const monthEntries = getDailyEntriesForMonth(period.period)
                              return monthEntries.map((entry: JurnalEntry) => {
                                const totalDebit = entry.details.reduce((sum: number, detail) => sum + detail.debit, 0)
                                const totalKredit = entry.details.reduce((sum: number, detail) => sum + detail.kredit, 0)
                                const isBalanced = totalDebit === totalKredit
                                  ...
                                  <TableRow key={entry.id} className="bg-muted/20">
                                    <TableCell className="pl-8 font-normal text-sm">
                                      <div className="flex flex-col">
                                        ...
                                        <span className="text-xs text-muted-foreground">{entry.nomorJurnal}</span>
                                      </div>
                                    </TableCell>
                                    <TableCell className="text-right text-sm">
                                      Rp {totalDebit.toLocaleString('id-ID')}
                              ...
                              const dayEntries = getEntriesForDay(period.period)
                              return dayEntries.map((entry) => {
                                ...
                                return (
                                  <TableRow key={entry.id} className="bg-muted/20">
                                    <TableCell className="pl-8 font-normal text-sm">
                                      <div className="flex flex-col">
                                        <span>{new Date(entry.tanggal).toLocaleTimeString('id-ID', {
                                          hour: '2-digit',
                                          minute: '2-digit'
                                        })}</span>
                                        <span className="text-xs text-muted-foreground">{entry.nomorJurnal}</span>
                    ...
                    <TableRow className="bg-muted/50 font-medium">
                      ...
                      <TableCell className="text-center">
                        <Badge variant={totalSummary.totalDebit === totalSummary.totalKredit ? "default" : "destructive"}>
                          {totalSummary.totalDebit === totalSummary.totalKredit ? "Seimbang" : "Tidak Seimbang"}
                        </Badge>
                      </TableCell>
                      <TableCell></TableCell>
                    </TableRow>
      ...
      <Dialog open={isDetailDialogOpen} onOpenChange={setIsDetailDialogOpen}>
        <DialogContent className="sm:max-w-[1000px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              Detail Jurnal - {selectedPeriodData ? formatPeriodLabel(selectedPeriodData.period) : ''}
            </DialogTitle>
            <DialogDescription>
          ...
          {selectedPeriodData && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4 p-4 bg-muted/50 rounded-lg">
                ...
                <div>
                  <Label className="text-sm font-medium text-muted-foreground">Total Kredit</Label>
                  <p className="text-lg font-semibold">
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold">{selectedPeriodData.transactionCount}</p>
                </div>
              </div>

              {/* Detailed Transactions */}
              <div className="space-y-4">
                <Label className="text-base font-medium">Detail Transaksi</Label>
                {loadingDetails ? (
                  <div className="flex items-center justify-center py-8">
                    <div className="text-sm text-muted-foreground">Memuat detail transaksi...</div>
                  </div>
                ) : detailedEntries.length === 0 ? (
                  <div className="text-center py-8">
                    <p className="text-sm text-muted-foreground">Tidak ada transaksi untuk periode ini</p>
                  </div>
                ) : (
                  <div className="border rounded-lg overflow-hidden">
                    <Table>
                        ...
                        {detailedEntries.map((entry) =>
                          entry.details.map((detail, index) => (
                            <TableRow key={`${entry.id}-${index}`}>
                              {index === 0 && (
                                <>
                                  <TableCell rowSpan={entry.details.length} className="font-medium">
                                    {new Date(entry.tanggal).toLocaleDateString('id-ID')}
                                  </TableCell>
                                  <TableCell rowSpan={entry.details.length} className="font-medium">
                                    {entry.nomorJurnal}
