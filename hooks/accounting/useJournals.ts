...
import { JurnalEntry, JurnalFormData, AccumulationData, AccumulationPeriod } from '@/types/accounting'
...
export function useJournals(options: UseJournalsOptions = {}) {
  ...
  const generateAccumulationData = useCallback(async (entries: JurnalEntry[], period: AccumulationPeriod) => {
    try {
      setAccumulationLoading(true)
      const data = await AccountingService.getAccumulationData(entries, period)
  ...
  const createEntry = useCallback(async (data: JurnalFormData): Promise<JurnalEntry | null> => {
    try {
      setLoading(true)
  ...
  const updateEntry = useCallback(async (id: string, data: Partial<JurnalFormData>): Promise<JurnalEntry | null> => {
    try {
      setLoading(true)
      const updatedEntry = await AccountingService.updateJournalEntry(id, data)
