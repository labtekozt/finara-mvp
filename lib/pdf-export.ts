...
export class JournalPDFExporter {
  static async exportRecapitulationReport(
    ...
    // Summary
    const totalDebit = accumulationData.reduce((sum, item) => sum + item.totalDebit, 0)
    const totalKredit = accumulationData.reduce((sum, item) => sum + item.totalKredit, 0)
    const totalTransactions = accumulationData.reduce((sum, item) => sum + item.transactionCount, 0)
    ...
    // Add table
    autoTable(pdf, {
      head: [['Periode', 'Total Debit', 'Total Kredit', 'Jumlah Transaksi', 'Status']],
      body: tableData,
      startY: 100,
      styles: {
        fontSize: 8,
    ...
    // Save the PDF
    pdf.save(`rekapitulasi-jurnal-${period}-${new Date().toISOString().split('T')[0]}.pdf`)
  }
  ...
  static async exportDetailedPeriodReport(
    ...
    // Period info
    pdf.setFontSize(12)
    pdf.text(`Periode: ${this.formatPeriodLabel(periodData.period, period)}`, 20, 35)
    pdf.text(`Total Debit: Rp ${periodData.totalDebit.toLocaleString('id-ID')}`, 20, 45)
    pdf.text(`Total Kredit: Rp ${periodData.totalKredit.toLocaleString('id-ID')}`, 20, 55)
    ...
    // Save the PDF
    pdf.save(`detail-jurnal-${periodData.period}-${new Date().toISOString().split('T')[0]}.pdf`)
  }
  ...
  private static formatPeriodLabel(periodStr: string, periodType: AccumulationPeriod): string {
    switch (periodType) {
      case 'daily':
